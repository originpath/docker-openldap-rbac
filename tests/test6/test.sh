...
RET=$?
if [ $RET -eq 0 ]; then
	ldapsearch -D "cn=test user,ou=People,dc=example,dc=com" -w test.user -H ldaps://ldap -b "cn=project-a-a,cn=project-a,ou=Groups,dc=example,dc=com" -s sub "objectclass=*" memberUid > output.txt
	RET=1
	NUM_EXPECTED=2
	LINES=$(grep -i 'memberUid:' output.txt | wc -l)
	grep -i 'memberUid:' output.txt
